...
function formatOpenAIRequest(messages, model, options = {}) {
  return {
    ...
    // Stop sequences to prevent repetition loops
    stop: ["___", "---"],
  };
}
...
async function chatWithConfig(messages, config, options = {}) {
  ...
  while (retries < 5) {
    response = await fetch(url, {
      ...
      signal: AbortSignal.timeout(600000), // 10 min timeout for local Ollama inference on CPU
    });
    
    if (response.status === 429) {
      const waitTime = Math.pow(2, retries) * 2000; // 2s, 4s, 8s, 16s...
      console.warn(`[LLM Client] ${provider} 429 Too Many Requests. Retrying in ${waitTime}ms (Attempt ${retries + 1}/5)...`);
  ...
  // Extract token usage (OpenAI format, supported by HF TGI)
  let tokens = { input: 0, output: 0 };
  if (data.usage) {
    tokens.input = data.usage.prompt_tokens || 0;
    tokens.output = data.usage.completion_tokens || 0;
  } else {
    // Fallback approximation
    const promptStr = typeof body === 'string' ? body : JSON.stringify(body);
    tokens.input = Math.ceil(promptStr.length / 4);
    tokens.output = Math.ceil((text || '').length / 4);
  }
 ...
 * @param {Array} messages - Array of { role, content }
 * @param {Object} options - Additional options
 * @returns {Promise<{ text: string, raw: Object, config: Object, usedFallback?: boolean }>}
 */
async function chat(messages, options = {}) {
  ...
  for (let i = 0; i < chain.length; i++) {
    ...
    try {
      const result = await chatWithConfig(messages, config, options);
      // Mark if we used fallback (i > 0 means not primary)
      if (i > 0) {
 ...
 * Uses judge config with fallback chain (judge -> primary -> fallback).
 * @param {Array} messages - Array of { role, content }
 * @param {Object} options - Additional options
 * @returns {Promise<{ text: string, raw: Object, config: Object }>}
 */
async function chatForJudge(messages, options = {}) {
  // Build judge fallback chain: judge -> primary -> fallback
  const configs = [];
  ...
  // Add primary and fallback if they're different from judge
  const chain = getModelChain();
  ...
  if (configs.length === 0) {
    throw new Error("No LLM models configured for judge. Check JUDGE_MODEL or PRIMARY_MODEL env vars.");
  }
  
  let lastError = null;
  
  // Try each config in the chain
  for (const config of configs) {
    try {
      // Judges typically need temperature 0 for consistency
      return await chatWithConfig(messages, config, { temperature: 0, ...options });
    } catch (err) {
      console.warn(`[LLM Client] Judge ${config.provider}/${config.model} failed:`, err.message);
      lastError = err;
      // Continue to next in chain
    }
  }
  
  throw new Error(`All judge providers failed. Last error: ${lastError?.message}`);
}
