...
explicitly exclude these profiles?
   Score 0 if: Multiple relevant contraindications exist in documents \
but none are mentioned.
   Score 4-6 if: Some contraindications mentioned but important ones \
omitted.
   Score 10 if: All relevant contraindications mentioned or genuinely \
not applicable.
...
- PARAPHRASED: meaning preserved but sentence structure or words are different (must tolerate typos and synonyms)
- INFERRED: follows logically from two or more statements in the documents
- UNSUPPORTED: absolutely no correspondence found in the documents — candidate for hallucination
...
IMPORTANT:
- Be highly tolerant of typos (e.g., "Paciêntes" instead of "pacientes") and synonyms.
- If the meaning is present in the DOCUMENTS, classify as PARAPHRASED, not UNSUPPORTED.
- List ONLY claims that are definitively UNSUPPORTED or HIGH RISK.
- Do not list every claim — focus on potential hallucinations.
- For each unsupported claim, you MUST fill the "search_process" field explaining why you couldn't find it (Chain of Thought).
- Return ONLY valid JSON, no additional text.
  ...
  "claims": [
    {
      ...
      "search_process": "Explain your search for this claim and why it is absolutely not in the documents",
