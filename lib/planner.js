...
async function planQuery(question, mode = "patient", history = [], extractedDrugs = []) {
  ...
  // Use the LLM-extracted drug if available, fallback to JS extractor.
  let drugName = classification.drug || (extractedDrugs.length > 0 ? extractedDrugs[0] : null);
  ...
  for (const tag of tags) {
    ...
    let { tool, section, fallback } = routing;
    
    // FarmaIA v2 Override: Se a Votação Q2Q descobrir a seção, use ela!
    if (classification.section_hint) {
      section = classification.section_hint;
      tool = 'get_section';
      console.log(`[Planner] V2 Q2Q Override: Forcing section to '${section}' based on vector voting.`);
    } else {
  ...
  if (!drugName || tags.length === 0) {
    ...
    plan = {
      ...
      classification_confidence: confidence,
      classification_method: classification.method,
      raw_tag: classification.raw_tag,
      autocorrect_similarity: classification.autocorrect_similarity,
