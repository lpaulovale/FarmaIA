...
async function tagAndFilter(text, section = "unknown", questionTags = []) {
  ...
  try {
    ...
    try {
      ...
      console.warn(`[Tagger] ${section} (${elapsed}ms): JSON parse failed, attempting automatic repair...`);
      let repaired = jsonMatch[0];
      // Tenta cortar depois da última chave fechada, caso a string tenha sido interrompida
      const lastBraceIndex = repaired.lastIndexOf('}');
      if (lastBraceIndex > 0) {
        repaired = repaired.substring(0, lastBraceIndex + 1);
        // Fecha array e objeto se estiverem faltando
        ...
        try {
          ...
          console.log(`[Tagger] JSON repaired successfully!`);
        } catch (e2) {
          throw parseErr; // Joga o erro original se o reparo falhar
        }
      } else {
        throw parseErr;
      }
