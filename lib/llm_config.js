...
function getApiKey(provider, specificKeyEnv) {
  ...
  const providerKeyMap = {
    ...
    groq: "GROQ_API_KEY",
    ollama: "OLLAMA_API_KEY", // Usually not needed, but good for consistency
...
function getModelConfig(purpose = "primary") {
  ...
  const provider = process.env[providerEnv]?.toLowerCase() || "groq";
  // Default model for Groq
  const model = process.env[modelEnv] || (provider === "groq" ? "llama-3.3-70b-versatile" : null);
  ...
  if (!providerConfig) {
    console.warn(`[LLM Config] Unknown provider '${provider}', falling back to groq`);
    return getModelConfigForProvider("groq", model, keyEnv);
  }

  const apiKey = getApiKey(provider, keyEnv);
  if (!apiKey) {
    // Only log if neither specific key nor provider-specific key (e.g., HF_TOKEN) is set
    return null;
  }
