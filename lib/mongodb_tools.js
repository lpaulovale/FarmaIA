...
async function searchMedication(query, bulaType = "paciente") {
  try {
    ...
    // Escape regex special characters to avoid crashes with e.g. "+"
    const escapedQuery = query.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

    // Case-insensitive regex search on name
    const docs = await coll.find({
      nome_medicamento: { $regex: escapedQuery, $options: "i" },
      tipo: "bula"
    }).limit(20).toArray();
...
/**
 * Helper to find the best match by sorting matches by name length (shortest first).
 * This prevents 'dipirona' from matching a long composite drug instead of 'dipirona sodica'.
 */
async function findBestMatch(coll, escapedDrugName) {
  // First, try exact match
  let doc = await coll.findOne({
    nome_medicamento: { $regex: new RegExp(`^${escapedDrugName}$`, "i") },
    tipo: "bula"
  });
  if (doc) return doc;

  // Then try partial match, fetch up to 20 and pick the shortest name
  let docs = await coll.find({
    nome_medicamento: { $regex: escapedDrugName, $options: "i" },
    tipo: "bula"
  }).limit(20).toArray();

  if (docs.length > 0) {
    return docs.sort((a, b) => a.nome_medicamento.length - b.nome_medicamento.length)[0];
  }

  // Try without accents
  const unaccented = escapedDrugName.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  docs = await coll.find({
    nome_medicamento: { $regex: unaccented, $options: "i" },
    tipo: "bula"
  }).limit(20).toArray();
...
async function getBulaData(drugName, mode = "patient") {
  try {
    ...
    const escapedDrugName = drugName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

    const doc = await findBestMatch(coll, escapedDrugName);
